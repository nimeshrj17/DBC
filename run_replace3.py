...
pattern = re.compile(
    r'\{\/\*\s*Side Panel Overlay\s*\*\/\}\s*\{selectedTable && \(\s*<div className="absolute inset-y-0 right-0 w-full max-w-md bg-card shadow-2xl border-l border-border flex flex-col transform transition-transform z-50">.*?\{selectedTable\.status === \'empty\' \? \([\s\S]*?\) \: \(',
...
if result == content:
    ...
    result = result.replace(old_end, new_end)
