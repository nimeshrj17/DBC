...
new_block = """{/* Side Panel Overlay */}
      {selectedTable && (
        <aside className={`absolute inset-y-0 right-0 h-full w-[430px] max-w-full flex-shrink-0 bg-white border-l border-slate-200 shadow-2xl flex flex-col justify-between transform transition-transform duration-300 z-50 slideover-shadow ${selectedTable ? 'translate-x-0' : 'translate-x-full'}`} data-purpose="table-detail-drawer">
          {/* Drawer Header */}
...
result = pattern.sub(new_block, content)
