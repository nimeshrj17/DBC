...
export function useOrders() {
  ...
  const createOrder = async (orderData: Omit<Order, 'id' | 'createdAt' | 'updatedAt' | 'displayId'> & { idempotencyKey?: string }) => {
    try {
      const displayId = `#KOT-${Math.floor(1000 + Math.random() * 9000)}`;
      const { idempotencyKey, ...dataToSave } = orderData;
        ...
        for (const invId of inventoryKeys) {
          ...
          if (invSnap.exists()) {
            ...
            if (currentQty - required <= 0) {
              ...
              // Note: Ideally we query this, but in transaction we can just update the ones we know about in this order
